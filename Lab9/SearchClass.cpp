...
int SearchClass::itBinarySearch(int searchItem){
	...
	while(first <= last) { 
        ...
        if(intArray[mid] == searchItem) return mid; 
        	...
        	if (searchItem < intArray[mid]) last = mid-1; 
            else first = mid+1; 
        } 
   	} 
    return -1; 
...
int SearchClass::binaryRecursion(int first, int last, int searchItem){
	...
	if(intArray[mid]==searchItem) return mid;
		...
		else if(intArray[mid] < searchItem) return binaryRecursion(mid+1,last,searchItem);
		else return binaryRecursion(first, mid-1, searchItem);	
		}
}
